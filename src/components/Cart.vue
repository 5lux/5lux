		...
		<div class="CartCenter">
			<ul class="CartCenterSection">
				...
				<li v-for="(n,i) in data">
					<div class="CartCenterTitle iconfont" v-if="data[i].length">{{n[0].brand_name}} &#x343c;</div>
					<ul>
						<li v-for="(value,index) in data[i]">
							...
							<img :src="data[i][index].good_img"/>
							<div class="CartCenterimport">
								<p>{{data[i][index].good_name}}</p>
								<p>{{data[i][index].good_name}}</p>
								<p>{{data[i][index].name2}}</p>
								<p>¥{{data[i][index].good_price}}</p>
								<div class="CartTodo" v-if="isEdit">
									<span @click="reduce(i,index)" :style="{color:data[i][index].good_number>1?'black':'#eee'}">-</span>
									<input type="text" :value="data[i][index].good_number"/>
	...
	export default {
		...
		mounted(){
			...
			for(var i=0;i<this.$store.state.cartProduct.length;i++){
				this.data.push([this.$store.state.cartProduct[i]])
			}
			...
			for(var i=0;i<this.data.length;i++){
				...
				for(var j=0;j<this.data[i].length;j++){
					...
					this.num += Number(this.data[i][j].good_number)
					this.sum += Number(this.data[i][j].good_number) * Number(this.data[i][j].good_price)
		...
		methods:{
			...
			reduce(i,j){
				if(Number(this.data[i][j].good_number)<=1){
					return 
				}else{
					this.data[i][j].good_number--
					this.sum -= Number(this.data[i][j].good_price)
				}
				this.isSelect = !this.isSelect
				this.isSelect = !this.isSelect
			},
